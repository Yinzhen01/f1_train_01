...
def validate_cloud_smoke(experiment, certificate, fingerprint, interrupted=False):
    ...
    if experiment.cfg.get('progress'):
        from .progress import validate_progress_certificate, validate_progress_diagnostics
        from .contact import validate_contact_diagnostics
        from .horizon import validate_horizon_probe
        validate_progress_certificate(experiment, certificate.get('continuation') or {})
        h = certificate.get('horizon_diagnostics') or {}
        validate_horizon_probe(h, 60., True)
