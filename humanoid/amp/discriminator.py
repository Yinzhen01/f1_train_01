...
class AMPDiscriminator(nn.Module):
    def __init__(self, spec, mean, std, hidden_dims=(256, 128)):
        super().__init__()
        self.spec = spec
    ...
    def losses(self, demo, policy, gradient_penalty=10.):
        if not len(demo) or not len(policy) or gradient_penalty < 0:
        ...
        return {"lsgan": lsgan, "gradient_penalty": penalty, "total": lsgan + penalty}
    ...
    def style_reward(self, windows, scale=1., dt=.01):
        ...
        return dt * scale * (1 - .25 * (score - 1).square()).clamp_min(0)


class DiscriminatorTrainer:
    def __init__(self, discriminator, learning_rate=1e-4, gradient_penalty=10., max_grad_norm=1.):
    ...
    def step(self, demo, policy):
        ...
        losses = self.discriminator.losses(demo, policy, self.gradient_penalty)
