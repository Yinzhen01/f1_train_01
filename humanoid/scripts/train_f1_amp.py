...
def main():
    ...
    def evaluate(checkpoint, completed_updates, smoke=False):
        ...
        extended = (horizoning or contacting or progressing or directing) and not smoke
    ...
    horizon_probe = HorizonProbe(env, smoke=extra.mode == 'smoke') if (horizoning or contacting or progressing or directing) else None
    if contacting or progressing or directing: env.reset_contact_diagnostics()
    if progressing: env.reset_progress_diagnostics()
    if directing: env.reset_direction_diagnostics()
    ...
    if horizon_probe is not None:
        ...
        validate_horizon_probe(certificate['horizon_diagnostics'], 60. if (contacting or progressing or directing) else experiment.cfg['horizon']['episode_length_s'],
                               extra.mode == 'smoke')
    if contacting or progressing or directing:
        certificate['contact_diagnostics'] = env.contact_diagnostics()
        validate_contact_diagnostics(certificate['contact_diagnostics'], 'tail' if (progressing or directing) else experiment.cfg['contact_refinement']['group'],
