...
from humanoid.gmr_swing_finetune import (SOURCE_TASK, SOURCE_SHA256, SOURCE_REFERENCE_SHA256,
...
def main():
    ...
    args = get_args()
    if args.task != "x1_gmr_swing" or not args.resume or args.checkpoint != 7000:
        raise ValueError("Requires x1_gmr_swing --resume --checkpoint=7000")
    ...
    if not 1 <= additional <= 1000:
        raise ValueError("This continuation is bounded to 1..1000 additional PPO updates")
    ...
    identity.update(code_commit=subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=LEGGED_GYM_ROOT_DIR).decode().strip(),
                    ...
                    final_completed_updates=7000 + additional, num_envs=env.num_envs,
                    learning_rate=runner.alg.learning_rate, schedule=runner.alg.schedule)
    print("[gmr-swing-resume] " + json.dumps(identity, sort_keys=True), flush=True)
    runner.learn(num_learning_iterations=additional, init_at_random_ep_len=False)
    print("[gmr-swing] training completed", flush=True)
