...
class TrainingProfilesTest(unittest.TestCase):
    ...
    def test_retarget_walk_native_geometry_resume_preserves_native_task(self):
        args = make_args(
        ...
        self.assertTrue(args.resume)
        self.assertEqual(args.load_run, "gm_resume")
        self.assertEqual(args.checkpoint, 1000)
        self.assertEqual(
            args.experiment_name,
            "x1_dh_stand_retarget_walk_periodic_contact_native_geometry",
        )
        self.assertEqual(args.max_iterations, 2000)

    def test_raw_root_speed_geometry_profile_is_independent_scratch_task(self):
        args = make_args(
            training_profile="retarget_walk_raw_root_speed_geometry_no_dr"
        )
        apply_training_profile(args)

        self.assertEqual(
            args.task, "x1_dh_stand_retarget_walk_raw_root_speed_geometry"
        )
        self.assertFalse(args.resume)
        self.assertEqual(
            args.experiment_name,
    ...
    def test_keypoints_posture_smooth001_resume_uses_explicit_checkpoint(self):
        args = make_args(
            training_profile=(
                "retarget_walk_keypoints_posture_smooth001_02547_resume"
            ),
        ...
        self.assertEqual(
            args.task,
            "x1_dh_stand_retarget_walk_keypoints_posture_smooth001_02547",
        )
        self.assertTrue(args.resume)
        self.assertEqual(args.load_run, "gm_resume")
