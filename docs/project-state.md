...
2026-09-11 终态核实：`TASK_20260911_112` 已于北京时间 16:02:52 正常完成，
最终 `model_5000.pt` 已上传、下载并核对内部迭代 4999 和 33 个有限网络张量。
当前仅适配推理身份检查、轨迹诊断和视频标签，不改变训练配置、参考或动力学。
73 项本地测试及真实训练 URDF 的 MuJoCo 渲染后端检查通过。
推理任务 `TASK_20260911_127` 已于 16:16:26 正常完成，产物已下载、解包并核实；
推理 commit 为 `bdad6c2cf89ab395e75ba4579df45b56e1d4e0d1`。三次均跑到 4.6 秒但不等于跟踪成功；
29 DOF 基本原地站立，未完成行走目标。
详见 `docs/gmr-policy-inference.md`。

本 worktree 于 2026-09-11 按用户要求创建为 `experiment/gmr-kit317-29dof`，
新增 `x1_gmr_29dof`：使用 GMR 对应的 F1 V1.4 29 关节模型和足底修正后的
完整 KIT317 动作，训练新的全身策略。不能从 12 关节 `model_5000.pt` 直接恢复。
已接入 29 关节观测、分组跟踪、胸部/手部监督和时间平滑奖励；69 项本地
测试通过，代码 `9688e5a` 已提交并推送。29 关节 Isaac Gym smoke
`TASK_20260911_111` 完成 20 次更新并上传 `model_20.pt`，运行时参数、
模型/数据哈希、奖励实际执行和模型张量有限性均已核对。
正式任务 `TASK_20260911_112`（2048 环境、5000 次更新、seed 5、从零训练、
4090D 24GB、无 DR）于 14:04:44 获平台启动受理，14:09:13 已实际推进到
日志 `23/5000`（1,179,648 timesteps）。实际 commit 为 `9688e5a`，
模型/参考哈希和全部运行时动力学与 smoke 完全一致，日志无数值异常。
上述 smoke 通过不代表收敛或抖动已改善；旧 12 关节平滑续训未启动。
上半身 PD 是待验证的仿真起始值。详见 `docs/gmr-kit317-29dof.md`。
下方 12 关节训练状态保留为分支来源历史，并非本实验实时状态。
