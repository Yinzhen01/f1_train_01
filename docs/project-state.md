...
2026-09-26：`experiment/f1-amp-sustain`已实现拟第17/18轮control/progress3，
同源089/heading2250完整恢复，唯一干预是前进权重2→3，各250更新至2500。
奖励复算发现停步后固定最终D的AMP奖励上升，不能只归因于平滑惩罚；未证明单一因果。
290项CPU回归通过；真实短测108(progress3)/109(control)均正常完成，10条更新日志、
完整源恢复、实际奖励调用与两类重置/独立回放核验通过，证书归档。
正式110(control)/111(progress3)已20:39启动，代码e317389，20:42日志2386/2373，
均原2250→2500、4096环境；效果待独立60秒回放验收。离线分析适配后291项回归通过，
未改变运行中的训练实现。正式已完成仍16/20，第17/18运行中，本轮之后最多剩2轮，
DR/noise关闭。统一协议与证据见[amp-sustain.md](amp-sustain.md)。下方为历史快照。
