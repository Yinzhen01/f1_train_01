...
更新：用户后续授权了仅缩放后的WALK_02独立AMP仿真实验，进展与奖励见
[单段实验](f1-amp-walk02-s092.md)和[当前状态](project-state.md)。
本文下方“未注册/未提交/未运行”等描述仅对应原十段准备阶段，不代表后续单段实验。
原十段数据验收状态及准备门禁保持不变，不能据单段短测宣称全部示范物理可执行。
