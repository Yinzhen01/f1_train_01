...
前向运动学得到的足底离地曲线，并在原曲线上提高 5%+5mm。左右支撑时序直接
读取 CSV 的软接触置信度；旧的固定接触数、腾空时间和 3–6cm 抬脚奖励仍关闭。

该任务只按名称读取左右髋、膝、踝共 12 个关节；CSV 中的浮动基座、腰、手臂、
颈部和头部不会进入参考动作。默认先在 nominal armature、无域随机化环境中验证
可训练性。146 帧参考周期使用 `26/146` 的相位偏移，使相位观测、基础约束和
数据接触标签对齐；0.45m/s 快速配置进一步启用参考足底高度奖励。配置细节见
