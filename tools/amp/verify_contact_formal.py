"""Verify bounded contact training and matched independent60 evaluation artifacts."""
import argparse
import hashlib
import json
...
from humanoid.amp.contact import validate_contact_certificate, validate_contact_diagnostics
...
def main():
    ...
    a = p.parse_args()
    if a.group not in ('control', 'slip', 'tail'): raise ValueError('Unknown contact group')
    ...
    validate_contact_certificate(e, m['continuation'])
    ...
    validate_contact_diagnostics(c['contact_diagnostics'], a.group, probe['control_steps'], 4096)
    assert m['runtime'] == source['runtime'], 'Changed runtime physics or PD'
    checkpoint = a.folder/'model_8802000.pt'
    digest = hashlib.sha256(checkpoint.read_bytes()).hexdigest()
    bundle_path = a.folder/'model_9902000.pt'
    ...
    assert baseline['checkpoint_sha256'] == e.cfg['contact_refinement']['source_checkpoint_sha256']
    ...
    final = torch.load(a.folder/'model_2000.pt', weights_only=True, map_location='cpu')
    assert state['completed_updates'] == final['completed_updates'] == 2000
    assert state['iter'] == final['iter'] == 1999
    ...
    validate_contact_updates(parse_updates(logs), formal=True)
    ...
    result = dict(task=a.task_id, group=a.group, verified=True, checkpoint_sha256=digest,
        ...
        completed_updates=2000, no_dr_verified=True, sdk_connection_reset_tracebacks=traces,
        initial_comparison=initial, horizon_diagnostics=probe, contact_diagnostics=c['contact_diagnostics'], modes=modes,
