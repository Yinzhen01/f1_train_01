"""Audit real progress smoke artifacts before accepting any formal run."""
import argparse
import hashlib
import json
...
from humanoid.amp.progress import validate_progress, validate_progress_diagnostics
...
def main():
    ...
    p.add_argument('--group', choices=('body', 'world'), required=True)
    ...
    validate_progress_diagnostics(cert['progress_diagnostics'], a.group, probe['control_steps'], 32)
    ...
    validate_progress_updates(parse_updates(logs))
    ...
    target = ROOT/'docs/validation'/('progress_'+a.group+'_cloud_smoke.json')
    ...
    print(json.dumps(dict(group=a.group, task=a.task_id, verified=True, checkpoint_sha256=digest,
        implementation_fingerprint=cert['implementation_fingerprint'], progress=cert['progress_diagnostics'], contact=cert['contact_diagnostics'], dr_unlocked=False)))
